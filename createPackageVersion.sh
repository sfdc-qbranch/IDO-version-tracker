#!/bin/bash
clear
DXPACKAGENAME="IDO Version Info"
...
then
  ...
  sfdx force:package:version:create  --installationkeybypass --definitionfile config/project-scratch-def.json --wait 10 --path ido-version --package "$DXPACKAGENAME" $@
fi
